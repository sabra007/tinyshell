...
void eval(char *cmdline) 
{
    ...
    if(!isBuiltIn) {
        ...
        if(!bg) {
            ...
            printf("%s: Command is running in the background.\n", argv[0]);
...
{
   /* if cmd is quit exit()
    * if cmd is jobs listjobs()
    * if cmd is bg or fg call do_bgfg()
    * otherwise return 0*/

    char * command = argv[0];
    //printf("%s: printing argv[0] from builtin_cmd.\n", command);
...
void sigtstp_handler(int sig) 
{
    int pid = fgpid(jobs);
    kill(-pid,SIGTSTP);
    printf("Job %d %d stopped", pid2jid(pid), pid);
    return;
}
